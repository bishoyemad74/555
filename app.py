...
# =========================================================
# 🇪🇬 نظام الوقت الموحد - توقيت القاهرة
# =========================================================
...
def egypt_date():
    """التاريخ الحالي بتوقيت القاهرة."""
    return egypt_now().strftime("%Y-%m-%d")


def egypt_datetime():
    """التاريخ والوقت الحالي بتوقيت القاهرة."""
    return egypt_now().strftime("%Y-%m-%d %H:%M:%S")


def egypt_time():
    """الساعة الحالية بتوقيت القاهرة."""
    return egypt_now().strftime("%H:%M:%S")


def egypt_timestamp():
    """Unix timestamp للوقت الحالي."""
    return egypt_now().timestamp()


# =========================================================
# 🔥 Firebase
# مهم: لا تضع Private Key داخل الكود.
# ضع بيانات Firebase في Streamlit Secrets باسم firebase_service_account
# =========================================================
...
def get_firebase_credentials():
    ...
    # دعم متغير البيئة كبديل
    raw = os.environ.get("FIREBASE_CREDS_JSON", "").strip()
    if raw:
        try:
...
if not firebase_admin._apps:
    if not firebase_creds:
        st.error(
            "❌ بيانات Firebase غير موجودة. أضف firebase_service_account "
            "إلى Streamlit Secrets أو FIREBASE_CREDS_JSON إلى Environment Variables."
        )
        st.stop()
...
def open_session_firebase(team_name, username):
    key_team = "team_1" if team_name == "الفريق الأول" else "team_2"

    # نستخدم timestamp حقيقي لحساب المدة، ووقت القاهرة للعرض.
    now_ts = egypt_timestamp()
    now_text = egypt_datetime()
...
def append_rows_to_google_sheet(sheet_name, rows_data):
    ...
    try:
        ...
        st.error(
            f"⚠️ خطأ في الرفع الجماعي ({sheet_name}): "
            f"{type(e).__name__} - {str(e)}"
        )
        ...
        if client:
            ...
            if records:
                ...
                for idx, row in df.iterrows():
                    if (
                        str(row.get("اسم المستخدم", "")).strip()
                        == str(username).strip()
                    ):
                        sheet.update_cell(
                            idx + 2,
                            2,
                            str(new_password).strip(),
                        )
                        st.cache_data.clear()
                        return True
    ...
    try:
        ...
        if not users_df.empty:
            ...
            if not user_match.empty:
                ...
                role = str(
                    row.get("الصلاحية", "مستخدم")
                ).strip()

                raw_perms = str(
                    row.get("القوائم المتاحة", "")
                ).strip()
                ...
                if role == "آدمن":
                    ...
                    permissions = {
                        ...
                        "can_directory": (
                            "الاعضاء" in raw_perms
                            or "دليل الكشافة" in raw_perms
                        ),
    ...
    with st.form("login_form"):
        ...
        if btn_login:
            if u_name.strip() and u_pass.strip():
                is_valid, role, perms = check_login(
                    u_name,
                    u_pass,
                )
                ...
                if is_valid:
                    ...
                    st.success(
                        f"مرحباً بك ({u_name})! جاري التحميل..."
                    )
                ...
                st.warning(
                    "يرجى كتابة اسم المستخدم وكلمة السر."
                )
...
# بيانات المستخدم
# =========================================================
col_user_info, col_pwd, col_logout = st.columns(
    [2.5, 1.2, 1]
)
...
# =========================================================
if (
    "members" not in st.session_state
    or st.session_state.members.empty
):
...
if "attendance" not in st.session_state:
    st.session_state.attendance = load_data_from_gsheet(
        "الحضور"
    )
...
if "scores" not in st.session_state:
    st.session_state.scores = load_data_from_gsheet(
        "التقييمات"
    )
...
if not available_tabs:
    st.warning(
        "⚠️ لا توجد صلاحيات لعرض أي قوائم."
    )
    st.stop()
...
if "attendance" in tab_dict:
    with tab_dict["attendance"]:
        ...
        if live_sessions:
            ...
            st.info(
                "ℹ️ لا توجد أي جلسات مفتوحة حالياً "
                "في أي فريق."
            )
        ...
        selected_key = (
            "team_1"
            if selected_team == "الفريق الأول"
            else "team_2"
        )

        active_session = live_sessions.get(
            selected_key,
            None,
        )
        ...
        if active_session:
            draft_scans = get_draft_scans_firebase(
                selected_team
            )
        ...
        with col_start:
            if st.button(
                ...
                if active_session:
                    m_user = active_session.get(
                        "user",
                        "قائد آخر",
                    )

                    m_time = active_session.get(
                        "start_time",
                        "",
                    )

                    st.error(
                        f"❌ عذراً! توجد جلسة مفتوحة بالفعل "
                        f"لـ ({selected_team}) قام بفتحها "
                        f"القائد ({m_user}) في ({m_time})."
                    )
                    ...
                    st.success(
                        f"🎉 تم بدء الجلسة لـ "
                        f"({selected_team}) بنجاح عبر Firebase!"
                    )
        ...
        with col_stop:
            if st.button(
                "🔴 إغلاق الجلسة وترحيل البيانات للسحاب فورا"
            ):
                if active_session:
                    try:
                        today = egypt_date()
                        ...
                        team_members = (
                            ...
                            if "الفريق"
                            in st.session_state.members.columns
                        ...
                        if rows_to_upload:
                            if append_rows_to_google_sheet(
                                "الحضور",
                                rows_to_upload,
                            ):
                                ...
                                close_session_firebase(
                                    selected_team
                                )

                                clear_draft_scans_firebase(
                                    selected_team
                                )

                                st.success(
                                    f"🎉 تم تسجيل حضور وغياب "
                                    f"({len(rows_to_upload)}) عضو "
                                    f"لـ ({selected_team}) بنجاح! ☁️"
                                )
                                ...
                                st.error(
                                    "❌ فشلت عملية الرفع الجماعي "
                                    "لـ Google Sheets."
                                )

                    except Exception as ex:
                        st.error(
                            f"❌ حدث خطأ غير متوقع أثناء "
                            f"إغلاق الجلسة: {ex}"
                        )
                        st.exception(ex)
                    ...
                    st.warning(
                        "لا توجد جلسة نشطة لهذا الفريق حالياً."
                    )
        ...
        if active_session:
            ...
            # Unix timestamps مستقلة عن timezone.
            # هذا يمنع اختلاف مدة الاجتماع بسبب المنطقة الزمنية.
            elapsed_seconds = max(
                0,
                egypt_timestamp() - start_ts,
            )
            ...
            st.info(
                f"⏱️ زمن الاجتماع: {elapsed_min} دقيقة | "
                f"درجة الحضور الآن: **{curr_score} / 10**"
            )
        ...
        if active_session:
            st.subheader(
                f"📷 التقاط الكارت والتسجيل التلقائي "
                f"({selected_team})"
            )

            img_file = st.camera_input(
                "اضغط التقاط الصورة لقرائتها وتسجيلها فوراً"
            )

            if img_file is not None:
                extracted = extract_qr_code(
                    img_file
                )
                ...
                if extracted:
                    ...
                    if not m.empty:
                        ...
                        if (
                            ...
                            st.info(
                                f"ℹ️ الكشاف {m_name} "
                                "مسجل بالفعل في هذه الجلسة."
                            )

                    else:
                        st.error(
                            f"❌ الكود ({clean_extracted}) "
                            f"غير مسجل ضمن أعضاء "
                            f"{selected_team}!"
                        )
            ...
            st.info(
                "💡 لا توجد جلسة مفتوحة لهذا الفريق. "
                "قم باختيار الفريق ثم اضغط "
                "**🚀 بدء الاجتماع / الجلسة**."
            )
        ...
        if active_session:
            with st.form(
                f"manual_attendance_form_"
                f"{st.session_state.manual_reset_counter}"
            ):
                manual_code_str = st.text_input(
                    "أو أدخل الكود يدوياً واضغط تسجيل:",
                    value="",
                )
...
if "evaluations" in tab_dict:
    with tab_dict["evaluations"]:
        ...
        with col_cam_btn:
            if st.button(
                "📷 فتح/إغلاق الكاميرا لمسح الكود",
                key="toggle_eval_cam_btn",
            ):
        ...
        if st.session_state.show_eval_camera:
            ...
            if eval_img is not None:
                extracted_eval = extract_qr_code(
                    eval_img
                )

                if extracted_eval:
                    st.session_state.eval_scanned_code = (
                        str(extracted_eval).strip()
                    )
        ...
        with st.form(
            f"score_form_"
            f"{st.session_state.eval_reset_counter}"
        ):
            ...
            submit_eval = st.form_submit_button(
                "حفظ التقييم والمزامنة السحابية"
            )
                ...
                if s_code_input.strip():
                    ...
                    if not matched.empty:
                        ...
                        if append_to_google_sheet(
                            ...
                            st.error(
                                "حدث خطأ أثناء الرفع السحابي."
                            )

                    else:
                        st.error(
                            "❌ الكود المدخل غير موجود في الأعضاء."
                        )
...
if "leaderboard" in tab_dict:
    with tab_dict["leaderboard"]:
        st.subheader(
            "🏆 ترتيب الكشافة حسب إجمالي الدرجات"
        )
        ...
        if not st.session_state.members.empty:
            members_df = (
                st.session_state.members.copy()
            )

            c_name = (
                "كود العضو"
                if "كود العضو"
                in members_df.columns
            ...
            att_df = (
                st.session_state.attendance.copy()
            )

            if (
                not att_df.empty
                and "كود العضو" in att_df.columns
            ):
                att_df["كود العضو"] = (
                    att_df["كود العضو"]
                    .astype(str)
            ...
            sc_df = (
                st.session_state.scores.copy()
            )

            if (
                not sc_df.empty
                and "كود العضو" in sc_df.columns
            ):
                sc_df["كود العضو"] = (
                    sc_df["كود العضو"]
                    .astype(str)
            ...
            leaderboard["المجموع الكلي"] = (
                leaderboard["نقاط الحضور"]
                + leaderboard["نقاط التقييمات"]
            )
        ...
        with col_sync:
            if st.button(
                "☁️ رفع ترتيب الأعضاء إلى Google Sheets"
            ):
                if not leaderboard.empty:
                    if update_leaderboard_in_gsheet(
                        leaderboard
                    ):
                        st.success(
                            "تم تحديث ورقة "
...
if "directory" in tab_dict:
    with tab_dict["directory"]:
        ...
        if gender == "ذكر":
            ...
            if (
                "إعدادي" in academic_stage
                or "ثانوي" in academic_stage
            ):
        ...
        m_dept = st.selectbox(
            ...
            index=default_idx,
            key=(
                f"widget_m_dept_{v}_"
                f"{gender}_{academic_stage}"
            ),
        )
...
if "sheet_link" in tab_dict:
    with tab_dict["sheet_link"]:
        st.subheader(
            "☁️ رابط Google Sheets المباشر"
        )

        st.markdown(
            f"[اضغط هنا لفتح Google Sheets "
            f"في نافذة جديدة]({SHEET_FULL_URL})"
        )

        st.markdown(
            f'<iframe src="{SHEET_FULL_URL}" '
...
if "accounts" in tab_dict:
    with tab_dict["accounts"]:
        st.subheader(
            "⚙️ إضافة حساب جديد وتحديد الصلاحيات"
        )
        ...
        users_list = load_data_from_gsheet(
            "المستخدمين"
        )
