...
def append_rows_to_google_sheet(sheet_name, rows_data):
    try:
        ...
        st.error(
            f"⚠️ خطأ في الرفع الجماعي ({sheet_name}): {type(e).__name__} - {str(e)}"
        )
    return False
...
with col_user_info:
    st.info(
        f"👤 **المستخدم:** {st.session_state.current_username} | **الصلاحية:** {st.session_state.user_role}"
    )
            ...
            if btn_save:
                if not old_p.strip() or not new_p1.strip():
                    ...
                    if verify_current_password(
                        st.session_state.current_username, old_p
                    ):
    ...
    with tab_dict["attendance"]:
        ...
        if live_sessions:
            for k_item, sess_v in live_sessions.items():
                ...
                st.success(
                    f"🟢 **جلسة نشطة حالياً لـ ({t_title})** | 👤 **القائد:** {u_owner} | 📅 **بدأت:** {st_time}"
                )
        else:
        ...
        with col_start:
            if st.button("🚀 بدء الاجتماع / الجلسة"):
                if active_session:
                    ...
                    st.error(
                        f"❌ عذراً! توجد جلسة مفتوحة بالفعل لـ ({selected_team}) قام بفتحها القائد ({m_user}) في ({m_time})."
                    )
                else:
                    now_ts = time.time()
                    now_egypt = datetime.datetime.now() + datetime.timedelta(
                        hours=3
                    )
            ...
            if st.button("🔴 إغلاق الجلسة وترحيل البيانات للسحاب فورا"):
                if active_session:
                    try:
                        ...
                        team_members = (
                            st.session_state.members[
                                st.session_state.members["الفريق"]
                                == selected_team
                            ]
                        ...
                        if rows_to_upload:
                            if append_rows_to_google_sheet(
                                ...
                                st.success(
                                    f"🎉 تم تسجيل حضور وغياب ({len(rows_to_upload)}) عضو لـ ({selected_team}) بنجاح! ☁️"
                                )
                                time.sleep(1)
                                st.rerun()
                            else:
                                st.error(
                                    "❌ فشلت عملية الرفع الجماعي لـ Google Sheets."
                                )
                    except Exception as ex:
                        st.error(
                            f"❌ حدث خطأ غير متوقع أثناء إغلاق الجلسة: {ex}"
                        )
                        st.exception(ex)
                else:
        ...
        if active_session:
            ...
            curr_score = max(
                0.0, round(10.0 - (int(elapsed_min // 5) * 1.0), 1)
            )
            st.info(
                f"⏱️ زمن الاجتماع: {elapsed_min} دقيقة | درجة الحضور الآن: **{curr_score} / 10**"
            )
        else:
            curr_score = 10.0
        ...
        if active_session:
            ...
            if scanned_qr_code:
                ...
                if not clean_extracted:
                    clean_extracted = str(scanned_qr_code).strip()
                ...
                if not m.empty:
                    ...
                    st.error(
                        f"❌ الكود ({clean_extracted}) غير مسجل ضمن أعضاء {selected_team}!"
                    )

        else:
            st.info(
                "💡 لا توجد جلسة مفتوحة لهذا الفريق. قم باختيار الفريق ثم اضغط **🚀 بدء الاجتماع / الجلسة**."
            )
                    ...
                    if manual_code_str.strip():
                        ...
                        if not m.empty:
                            ...
                            if clean_manual not in scanned_members:
                                ...
                                st.success(
                                    f"🎉 تم تسجيل: {m_name} ({selected_team}) | الدرجة: {curr_score}/10"
                                )
                                time.sleep(0.8)
                                st.rerun()
                            else:
...
if "leaderboard" in tab_dict:
    with tab_dict["leaderboard"]:
        ...
        if not st.session_state.members.empty:
            ...
            t_name = (
                "الفريق" if "الفريق" in members_df.columns else "الفريق"
            )
            ...
            if not att_df.empty and "كود العضو" in att_df.columns:
                ...
                att_sum = (
                    att_df.groupby("كود العضو")["درجة الحضور"]
                    .sum()
                    .reset_index()
                    if "درجة الحضور" in att_df.columns
            ...
            if not sc_df.empty and "كود العضو" in sc_df.columns:
                ...
                sc_df[sc_v] = pd.to_numeric(sc_df[sc_v], errors="coerce").fillna(
                    0
                )
            ...
            leaderboard = leaderboard.merge(
                att_sum, on=c_name, how="left"
            ).fillna(0)
            leaderboard = leaderboard.merge(
                sc_sum, on=c_name, how="left"
            ).fillna(0)
            ...
            if st.button("☁️ رفع ترتيب الأعضاء إلى Google Sheets"):
                if not leaderboard.empty:
                    if update_leaderboard_in_gsheet(leaderboard):
                        st.success(
                            "تم تحديث ورقة 'ترتيب الأعضاء' سحابياً بنجاح! 🎉"
                        )
    ...
    with tab_dict["accounts"]:
        ...
        with st.form("add_user_form", clear_on_submit=True):
            new_u_name = st.text_input(
                "اسم المستخدم الجديد", key="input_u_name"
            )
