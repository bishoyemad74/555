...
# --- جلب البيانات مع دعم الكاش لتحديث الجلسات بين الأجهزة ---
@st.cache_data(ttl=3)
def fetch_gsheet_cached(sheet_name):
    try:
        client = get_gsheet_client()
        if client:
            sheet = client.open_by_key(SPREADSHEET_ID).worksheet(sheet_name)
...
def load_data_from_gsheet(sheet_name, use_cache=False):
    if use_cache:
        return fetch_gsheet_cached(sheet_name)
    try:
        client = get_gsheet_client()
        if client:
            sheet = client.open_by_key(SPREADSHEET_ID).worksheet(sheet_name)
...
# --- Tab: تسجيل الحضور (معدل لدعم الفريقين والمزامنة السحابية الحية) ---
if "attendance" in tab_dict:
    with tab_dict["attendance"]:
        st.subheader("تسجيل الحضور الفوري")
        
        # اختيار الفريق للجلسة
        selected_team = st.selectbox("🚩 اختر الفريق لعقد الجلسة:", ["الفريق الأول", "الفريق الثاني"], key="team_selector")

        # مزامنة سحابية حية للجلسات عبر الكاش
        session_info = load_data_from_gsheet("حالة_الجلسة", use_cache=True)
        active_session = None

        if not session_info.empty and "الحالة" in session_info.columns:
            # البحث عن جلسة مفتوحة تخص الفريق المحدد
            open_rows = session_info[(session_info["الحالة"] == "مفتوحة") & (session_info["الفريق"] == selected_team)]
        ...
        if active_session:
            st.warning(f"⚠️ هناك جلسة مفتوحة لـ ({selected_team}) بدأها القائد ({active_session.get('المستخدم', 'غير معروف')}) بتاريخ {active_session.get('التاريخ', '')}")
            
            # مزامنة مسودة الحضور للفريق المحدد
            draft_df = load_data_from_gsheet("مسودة_الحضور", use_cache=True)
            st.session_state.scanned_members = {}
            if not draft_df.empty and "كود العضو" in draft_df.columns:
                team_draft = draft_df[draft_df["الفريق"] == selected_team] if "الفريق" in draft_df.columns else draft_df
                for _, d_row in team_draft.iterrows():
        ...
        with col_start:
            if st.button(f"🚀 بدء اجتماع ({selected_team})"):
                now_ts = time.time()
                today_date = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
                st.session_state.session_start_time = now_ts
                st.session_state.scanned_members = {}
                
                new_sess_row = [today_date, st.session_state.current_username, "مفتوحة", str(now_ts), selected_team]
                append_to_google_sheet("حالة_الجلسة", new_sess_row)
                st.cache_data.clear()
                
                st.success(f"بدأت الجلسة السحابية لـ ({selected_team})! الدرجة الحالية 10/10.")
                st.rerun()
                
        with col_stop:
            if st.button(f"🔴 إغلاق جلسة ({selected_team}) وترحيل البيانات"):
                if active_session or st.session_state.session_start_time is not None:
                    ...
                    for _, row in st.session_state.members.iterrows():
                        ...
                        row_data = [today, c, n, st_name, t_str, sc, selected_team]
                        append_to_google_sheet("الحضور", row_data)
                        
                        new_att.append({
                            "التاريخ": today, "كود العضو": c, "اسم الكشاف": n,
                            "حالة الحضور": st_name, "وقت التسجيل": t_str, "درجة الحضور": sc, "الفريق": selected_team
                        })
                    ...
                    st.success(f"تم إغلاق جلسة ({selected_team}) وترحيل البيانات مباشرة لـ Google Sheets! ☁️")
                    st.rerun()
                else:
                    st.warning(f"لا توجد جلسة نشطة لـ ({selected_team}) حالياً.")
        ...
        if st.session_state.session_start_time is not None:
            ...
            st.info(f"⏱️ زمن اجتماع ({selected_team}): {elapsed_min} دقيقة | درجة الحضور الآن: **{curr_score} / 10**")
        else:
            curr_score = 10.0
        ...
        if st.session_state.session_start_time is not None:
                    ...
                    try:
                        ...
                        if not m.empty:
                            ...
                            if code_val not in st.session_state.scanned_members:
                                ...
                                draft_row = [code_val, m_name, t_now, curr_score, st.session_state.current_username, selected_team]
                                append_to_google_sheet("مسودة_الحضور", draft_row)
                                st.cache_data.clear()
                                
                                st.success(f"🎉 تم قراءة الكود وتسجيل حضور {m_name} في ({selected_team})")
                                st.balloons()
                            else:
            ...
            st.info(f"💡 اضغط على زر **🚀 بدء اجتماع ({selected_team})** أعلاه لفتح الكاميرا وبدء التسجيل.")
                    ...
                    try:
                        ...
                        if not m.empty:
                            ...
                            if manual_code not in st.session_state.scanned_members:
                                ...
                                draft_row = [manual_code, m_name, t_now, curr_score, st.session_state.current_username, selected_team]
                                append_to_google_sheet("مسودة_الحضور", draft_row)
                                st.cache_data.clear()
                                
                                st.session_state.manual_reset_counter += 1
                                st.success(f"🎉 تم تسجيل: {m_name} في ({selected_team}) | الدرجة: {curr_score}/10")
                                time.sleep(0.8)
                                st.rerun()
                            else:
...
if "directory" in tab_dict:
    with tab_dict["directory"]:
        ...
        default_depts = ["براعم", "أشبال", "زهرات", "كشاف", "مرشدات", "متقدم", "رائدات", "جوال", "جوالات", "قادة"]
